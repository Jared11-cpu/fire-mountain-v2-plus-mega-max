...
function Stops({ points, selectedId, fallbackImageUrl, dailyRecords, maxDays, onSelect, onPatchPoint, notes, onPatchNote }: { points: PlannedRoutePoint[]; selectedId?: string; fallbackImageUrl: string; dailyRecords: Array<{ day: number; date: string }>; maxDays: number; onSelect: (point: RoutePoint) => void; onPatchPoint: (id: string, changes: Partial<PlannedRoutePoint>) => void; notes: Record<string, string>; onPatchNote: (id: string, note: string) => void }) {
  ...
  return <div className="space-y-4"><h4 className="font-display text-2xl font-black">地点安排</h4>{points.map((point, index) => { const expanded = expandedId === point.id; const date = dailyRecords.find((record) => record.day === (point.day ?? 1))?.date; const serviceLinks = getPointServiceLinks(point); const resolvedCover = getCuratedPointCover(point.name) ?? fetchedCovers[point.id]; const coverUrl = resolvedCover?.imageUrl ?? point.imageUrl ?? fallbackImageUrl; const coverCredit = resolvedCover?.imageCredit ?? point.imageCredit; return <article key={point.id} className={`overflow-hidden rounded-[1.65rem] border bg-white transition ${selectedId === point.id ? 'border-river shadow-[0_12px_35px_rgba(14,116,128,.14)]' : 'border-ink/10 shadow-sm'}`}>
      ...
      {expanded && <div className="space-y-4 p-4">
        ...
        <div className="flex items-center justify-between gap-3">{coverCredit ? <a href={coverCredit.sourceUrl} target="_blank" rel="noreferrer" className="text-[10px] font-bold text-ink/40 underline decoration-ink/20 underline-offset-2">{coverCredit.author} · {coverCredit.license}</a> : <span className="text-[10px] font-bold text-ink/40">地点实景图</span>}<a href={serviceLinks.detailUrl} target="_blank" rel="noreferrer" aria-label={`${point.name}地点详细信息`} className="inline-flex shrink-0 items-center gap-1 rounded-full bg-ink px-3 py-2 text-xs font-black text-white transition hover:bg-river">地点详细信息<ExternalLink className="h-3.5 w-3.5" /></a></div>
      </div>}
    </article>; })}</div>;
}
...
export function getBudgetUsageVisual(actual: number, planned: number) {
  ...
  const hue = Math.round(145 * (1 - clampedPercent / 100));
  return {
    percent,
    clampedPercent,
...
function Budget({ items, target, days, onChange }: { items: BudgetItem[]; target: number; days: number; onChange: (items: BudgetItem[]) => void }) {
  ...
  const ringDegrees = usage.clampedPercent * 3.6;
  const ringBackground = `conic-gradient(from -90deg, hsl(145 78% 55%) 0deg, ${usage.color} ${ringDegrees}deg, rgba(255,255,255,.12) ${ringDegrees}deg 360deg)`;
  return <div className="space-y-4"><div><h4 className="font-display text-2xl font-black">旅行预算</h4><p className="mt-1 text-xs font-bold text-ink/45">金额可直接选中输入，所有条目自动保存。</p></div><section className="relative overflow-hidden rounded-[1.9rem] bg-gradient-to-br from-ink via-[#173b47] to-river p-5 text-white shadow-[0_22px_54px_rgba(18,34,42,.22)]"><WalletCards className="absolute -right-5 -top-6 h-32 w-32 rotate-[-12deg] text-white/[0.055]" /><div className="relative flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-[0.26em] text-[#62e6c1]">PLAN BUDGET</span><div className={`rounded-full border px-3 py-1 text-[10px] font-black ${remaining >= 0 ? 'border-emerald-300/20 bg-emerald-300/10 text-emerald-100' : 'border-orange-200/20 bg-orange-300/15 text-orange-100'}`}>{remaining >= 0 ? '预算内' : '已超支'}</div></div><div className="relative mt-4 grid grid-cols-2 divide-x divide-white/10"><BudgetAmount label="计划预算" value={target} /><BudgetAmount label="实际花费" value={total} align="right" /></div><div className="relative mt-5 flex items-center gap-5 rounded-[1.55rem] border border-white/10 bg-white/[0.055] p-4"><div aria-label={`预算已使用 ${usage.percent}%`} className="grid h-[108px] w-[108px] shrink-0 place-items-center rounded-full p-[9px] shadow-[0_12px_28px_rgba(0,0,0,.16)] transition-all duration-500" style={{ background: ringBackground }}><div className="grid h-full w-full place-items-center rounded-full bg-[#173843] text-center shadow-inner"><div><strong className="block font-display text-2xl font-black leading-none" style={{ color: usage.color }}>{usage.percent}%</strong><span className="mt-1.5 block text-[10px] font-black tracking-[0.16em] text-white/50">已使用</span></div></div></div><div className="min-w-0"><span className="text-[10px] font-black uppercase tracking-[0.18em] text-white/40">ACTUAL / PLAN</span><p className="mt-2 font-display text-lg font-black leading-snug">实际支出占计划预算</p><p className="mt-1 text-[11px] font-bold leading-5 text-white/45">使用率越高，环形由绿色渐变为黄色、红色。</p></div></div><div className="relative mt-4 grid grid-cols-3 gap-2.5"><BudgetFact label="剩余额度" value={`${remaining < 0 ? '-' : ''}¥${Math.abs(remaining).toLocaleString('zh-CN')}`} tone={remaining < 0 ? 'warn' : 'normal'} /><BudgetFact label="日均预算" value={`¥${Math.round(target / Math.max(1, days)).toLocaleString('zh-CN')}`} /><BudgetFact label="预算条目" value={`${items.length} 项`} /></div></section>
...
function BudgetAmount({ label, value, align = 'left' }: { label: string; value: number; align?: 'left' | 'right' }) { return <div className={align === 'right' ? 'pl-5 text-right' : 'pr-5'}><span className="block text-[11px] font-black tracking-[0.08em] text-white/45">{label}</span><strong className="mt-1 block font-display text-[2rem] font-black leading-none tracking-[-0.03em]">¥{value.toLocaleString('zh-CN')}</strong></div>; }
function BudgetFact({ label, value, tone = 'normal' }: { label: string; value: string; tone?: 'normal' | 'warn' }) { return <div className="rounded-2xl border border-white/10 bg-white/[0.07] px-3 py-3.5"><span className="block text-[10px] font-black tracking-[0.05em] text-white/42">{label}</span><strong className={`mt-1.5 block font-display text-base font-black ${tone === 'warn' ? 'text-orange-200' : 'text-white'}`}>{value}</strong></div>; }
