...
export function MapWorkspace({ route, plan, selectedPointId, activePointIndex, navigating, imageUrl, onSelectPoint, onRegenerate, onSimulateNavigation }: {
  ...
  const loadTransport = useCallback((signal?: AbortSignal, silent = false) => {
    ...
    return resolveTransportComparison(toTransportPlanRequest(request, tripPlan?.route.points as PlannedRoutePoint[] ?? [], tripPlan?.settings.departureTime ?? '08:30', transportStrategy), { signal })
      .then((result) => { setTransportComparison(result); setTransportChoiceId(result.recommendedOptionId); })
    ...
    <div className="grid lg:h-[calc(100vh-210px)] lg:min-h-[700px] lg:grid-cols-[72px_minmax(0,1fr)_420px]">
      ...
      <aside aria-label="方案详情" className={`${mobilePane === 'details' ? 'flex' : 'hidden'} workspace-detail-glass min-h-0 min-w-0 flex-col overflow-hidden lg:flex`}>
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-5 [scrollbar-gutter:stable]">
          ...
          {tab === 'transport' && <Transport comparison={transportComparison} selectedId={transportChoiceId} loading={transportLoading} strategy={transportStrategy} onSelect={setTransportChoiceId} onStrategy={setTransportStrategy} onReload={() => loadTransport().catch(() => undefined)} onSimulate={onSimulateNavigation} />}
...
function Transport({ comparison, selectedId, loading, strategy, onSelect, onStrategy, onReload, onSimulate }: { comparison: TransportComparison | null; selectedId: TransportChoiceId; loading: boolean; strategy: TransitStrategy; onSelect: (id: TransportChoiceId) => void; onStrategy: (strategy: TransitStrategy) => void; onReload: () => void; onSimulate?: () => void }) {
  ...
  return <div className="space-y-4"><div className="flex items-end justify-between gap-3"><div><h4 className="font-display text-2xl font-black">交通方案</h4><p className="mt-1 text-xs font-bold text-ink/45">高德路线 · 千问推荐</p></div><button type="button" aria-label="重新查询交通方案" disabled={loading} onClick={onReload} className="grid h-9 w-9 place-items-center rounded-full bg-white text-river shadow-sm disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button></div>
    ...
    {selectedId === 'transit' && <div className="flex gap-2 overflow-x-auto pb-1" role="radiogroup" aria-label="公共交通路线偏好">{transitStrategies.map((item) => <button key={item.id} type="button" role="radio" aria-checked={strategy === item.id} onClick={() => onStrategy(item.id)} className={`shrink-0 rounded-full px-3 py-2 text-[10px] font-black transition ${strategy === item.id ? 'bg-ink text-white shadow-sm' : 'bg-white text-ink/50'}`}>{item.label}</button>)}</div>}
    <section className="overflow-hidden rounded-[1.6rem] bg-ink text-white shadow-[0_16px_38px_rgba(18,34,42,.16)]"><div className="p-4"><div className="flex items-center justify-between gap-3"><span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-black ${freshness !== 'estimate' ? 'bg-jade/25 text-emerald-100' : 'bg-white/10 text-white/65'}`}>{freshness !== 'estimate' ? <Wifi className="h-3.5 w-3.5" /> : <WifiOff className="h-3.5 w-3.5" />}{transport?.sourceLabel ?? '正在查询'}</span><span className="text-[9px] font-bold text-white/45">{statusCopy}</span></div><p className="mt-4 text-sm font-bold leading-6 text-white/75">{transport?.summary ?? '正在根据当前出发时间查询交通方案……'}</p><div className="mt-4 grid grid-cols-3 gap-2"><TransportKpi label="总时长" value={transport ? `${transport.totalMinutes} 分` : '—'} /><TransportKpi label="总距离" value={transport ? `${transport.totalDistanceKm} km` : '—'} /><TransportKpi label={selectedId === 'driving' ? '过路费' : '预计票价'} value={transport?.totalFare === undefined ? '待查询' : `¥${transport.totalFare}`} /></div></div><div className="border-t border-white/10 bg-white/5 px-4 py-3 text-[10px] font-bold leading-4 text-white/50">{transport ? `更新于 ${new Date(transport.generatedAt).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })} · ${statusCopy}` : '正在连接交通服务'}</div></section>
...
function Budget({ items, target, days, onChange }: { items: BudgetItem[]; target: number; days: number; onChange: (items: BudgetItem[]) => void }) {
  ...
  const unfilled = 100 - usage.clampedPercent;
  return <div className="space-y-4"><div><h4 className="font-display text-2xl font-black">旅行预算</h4><p className="mt-1 text-xs font-bold text-ink/45">计划预算在概览修改；这里只记录实际支出。</p></div><section aria-label={`实际花费占计划预算 ${usage.percent}%`} className="relative overflow-hidden rounded-[1.9rem] bg-[linear-gradient(90deg,#15966a_0%,#dfb42b_58%,#d94136_100%)] p-5 text-white shadow-[0_22px_54px_rgba(18,34,42,.22)]"><div aria-hidden="true" className="absolute inset-y-0 right-0 bg-[#173943] transition-[width] duration-700 ease-out" style={{ width: `${unfilled}%` }} /><div aria-hidden="true" className="absolute inset-0 bg-gradient-to-br from-black/5 via-transparent to-black/20" /><div className="relative grid grid-cols-2 gap-5"><BudgetAmount label="计划" value={target} /><BudgetAmount label="实际" value={total} align="right" /></div><div className="relative mt-7 flex items-end justify-between gap-4"><div><strong className="font-display text-[3.25rem] font-black leading-none tracking-[-0.05em]">{usage.percent}%</strong><span className="ml-2 text-xs font-black text-white/70">已花费</span></div><span className="rounded-full border border-white/15 bg-black/10 px-3 py-1.5 text-[10px] font-black backdrop-blur">{remaining >= 0 ? `剩余 ¥${remaining.toLocaleString('zh-CN')}` : `超出 ¥${Math.abs(remaining).toLocaleString('zh-CN')}`}</span></div><div className="relative mt-5 h-1.5 overflow-hidden rounded-full bg-black/20"><div className="h-full rounded-full bg-white/85 shadow-[0_0_14px_rgba(255,255,255,.5)] transition-[width] duration-700" style={{ width: `${usage.clampedPercent}%` }} /></div><div className="relative mt-4 grid grid-cols-3 border-t border-white/15 pt-4"><BudgetFact label="剩余" value={`${remaining < 0 ? '-' : ''}¥${Math.abs(remaining).toLocaleString('zh-CN')}`} tone={remaining < 0 ? 'warn' : 'normal'} /><BudgetFact label="日均" value={`¥${Math.round(target / Math.max(1, days)).toLocaleString('zh-CN')}`} /><BudgetFact label="条目" value={`${items.length} 项`} /></div></section>
